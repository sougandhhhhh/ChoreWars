...
export default function ProfilePage() {
  ...
  const getDailyActivities = () => {
    ...
    history.forEach(log => {
      if (log.loggerId === pid || log.helperIds?.includes(pid)) {
        if (log.choreId === "reward-failed" || log.choreId === "extra" || log.choreId === "reward") return;
        const logDate = new Date(log.timestamp);
        if (logDate.getFullYear() === currentDate.getFullYear() && logDate.getMonth() === currentDate.getMonth()) {
          const day = logDate.getDate();
          if (!activities[day]) activities[day] = { chores: [], count: 0 };
  ...
  const getProfile = (id: string) => {
    ...
    return { ...base, profileId: id, name: overrides.name || base.name, codename: overrides.codename || base.codename };
  };

  const getRank = (cid: string, targetPid: string) => {
    const scores = PEOPLE.map(id => history.filter(h => {
  ...
  const COUNTRY_CODES = [
    { code: "+1", label: "US/CA (+1)" }, { code: "+44", label: "UK (+44)" }, { code: "+91", label: "IN (+91)" },
    { code: "+61", label: "AU (+61)" }, { code: "+81", label: "JP (+81)" }, { code: "+49", label: "DE (+49)" },
    { code: "+33", label: "FR (+33)" }, { code: "+971", label: "AE (+971)" },
  ]
